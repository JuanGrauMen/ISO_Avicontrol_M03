...
**Independent Test**: Se selecciona un galpón en estado `En Cosecha`, se ingresan pollos vendidos, peso promedio y precio/kg válidos, y se verifica que la venta queda guardada con fecha, hora, responsable y asociada al lote, bloqueando registros duplicados.

**Acceptance Scenarios**:

1. **Scenario**: Registro exitoso de matriz de ventas al cierre
   - **Given** un galpón en estado `En Cosecha` con población viva reportada por el Módulo 1 y sin matriz de ventas activa previa
...
2. **Scenario**: Rechazo por cantidad de pollos vendidos inválida
   - **Given** un galpón seleccionado con población viva de 8.500 pollos según Módulo 1
...
- **FR-002**: El sistema MUST validar que la cantidad de pollos vendidos cumpla la regla de negocio `1 ≤ pollos vendidos ≤ población viva actual (Módulo 1)`.
...
- **FR-005**: El sistema MUST asociar de forma inmutable a cada registro de venta: identificador de lote, fecha y hora de registro, y usuario responsable.
...
- **FR-008**: Si la población reportada por Módulo 2 difiere de la de Módulo 1 al momento del registro, el sistema MUST utilizar la de Módulo 1 (fuente oficial) y desplegar una advertencia informativa no bloqueante.

### Key Entities

- **MatrizVentas**: Registro de la transacción comercial del lote. Atributos clave: `idVenta`, `idLote`, `pollosVendidos`, `pesoPromedioKg`, `precioKgCop`, `estado` (`ACTIVA` | `ANULADA`), `fechaRegistro`, `usuarioResponsable`.
...
- **SC-001**: El 100% de las ventas registradas cumplen estrictamente el rango de población viva del Módulo 1 y valores mayores a cero.
