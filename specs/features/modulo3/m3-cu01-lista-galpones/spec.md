...
2. **Scenario**: Selección de galpón en cosecha para flujo de ventas/liquidación
   ...
   - **Then** el sistema habilita la opción para registrar matriz de ventas (M3-CU02) y, si existe una matriz de ventas `ACTIVA`, habilita la opción para generar liquidación (M3-CU03)

3. **Scenario**: Galpón sin condiciones para venta o liquidación
   - **Given** un galpón listado en estado `Disponible`, `Vaciado Sanitario`, `Productivo`, `Mantenimiento` o `Aislamiento`
   - **When** el usuario interactúa con dicho galpón
   - **Then** el sistema muestra su información de forma informativa pero mantiene deshabilitadas las acciones de registro de venta y generación de liquidación, indicando que el galpón no se encuentra en fase de cosecha
...
5. **Scenario**: Indisponibilidad temporal de Módulo 1 durante la sincronización
...
- **FR-003**: El sistema MUST admitir únicamente el siguiente catálogo unificado de estados operativos:
  ...
  - `En Cosecha`: Lote en fase de recolección y venta (accionable para registro de ventas y, con venta activa, liquidación).
...
- **FR-004**: El sistema MUST permitir al usuario seleccionar un galpón en estado `En Cosecha` para navegar hacia el registro de matriz de ventas (M3-CU02) y, si existe matriz de ventas `ACTIVA`, hacia la liquidación del lote (M3-CU03).
- **FR-005**: El sistema MUST deshabilitar los botones de registro de venta y generación de liquidación para galpones en estados `Disponible`, `Vaciado Sanitario`, `Productivo`, `Mantenimiento` o `Aislamiento`.
...
- **SC-003**: Cero accesos permitidos a formularios de venta o liquidación sobre galpones en estado diferente de `En Cosecha`.
