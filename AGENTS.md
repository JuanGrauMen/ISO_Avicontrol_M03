...
3. `docs/specs/features/modulo3/plan/plan-v2.md` — **plan técnico general** del módulo, con la estructura de `plan-template.md`: estructura de código, tareas por CU (con sus tests y supuestos sobre D-01..D-07) y orden de ejecución. Define el *cómo*; nunca contradice a un spec.
...
Cada CU tiene su carpeta y su `spec.md` es autocontenido. **No existe M3-CU02**: se unificó en CU03 el 2026-09-21 (ver `CAMBIOS.md` §7); el ID queda vacante a propósito. El PLAN es **uno solo para el módulo** (carpeta `plan/`, decidido el 2026-09-21; vigente `plan-v2.md`; cada revisión se agrega como archivo nuevo `plan-vN.md` y manda la de número más alto): cubre los 9 CU con una fase de tareas por cada uno. No se crean planes por CU. En el resto de este archivo, `plan.md` significa la versión vigente del plan.
...
2. **PLAN** — leer la fase del CU en `docs/specs/features/modulo3/plan/plan-v2.md` (plan general escrito a partir de `docs/specs/templates/plan-template.md`). Ahí están las clases a crear (árbol de *Project Structure*), las tareas de la fase con el mapa acceptance scenario → `@Test`, los FR que cubre y los supuestos sobre decisiones abiertas. Si al implementar el plan resulta insuficiente o incorrecto para ese CU, se corrige el `plan.md` (y se revisa con el equipo) antes de seguir; no se improvisa en código.
...
- [ ] La fase del CU en `plan.md` está completa (tareas `T0NN` tachadas) y el código cubre todos los FR que su *Goal* declara.
