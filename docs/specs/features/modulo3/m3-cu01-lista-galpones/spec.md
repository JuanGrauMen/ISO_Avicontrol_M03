...
Como administrador financiero, quiero consultar la lista consolidada de galpones y sus lotes con su identificación, nombre y estado operativo, para identificar la situación de cada lote y seleccionar los que estén en condiciones de generar una Liquidación.
...
3. **Scenario**: Selección de galpón en vaciado sanitario para operar
   ...
   - **Then** el sistema habilita la generación de la Liquidación (M3-CU03) si existe un resultado final de sacrificio sincronizado desde Módulo 2 o si la población actual sincronizada es 0 por mortalidad total, y no existe Liquidación `ACTIVA`
...
4. **Scenario**: Galpón sin condiciones para operar
   ...
   - **When** el usuario interactúa con dicho galpón
   - **Then** el sistema muestra su información de forma informativa pero mantiene deshabilitada la generación de la Liquidación, indicando que el galpón no se encuentra en fase de cosecha ni de vaciado sanitario
...
5. **Scenario**: Ausencia total de datos sincronizados
   ...
   - **Then** el sistema presenta un estado vacío con el mensaje "No hay galpones disponibles para consultar" y bloquea cualquier intento de generar una Liquidación
...
- **FR-004**: Cuando el estado sea `Vaciado Sanitario`, el sistema MUST identificar el lote mediante los UUID históricos de la alerta de vaciado sanitario. MUST ofrecer, **en la propia fila del galpón y sin pantalla de detalle intermedia**, la acción de generar la Liquidación (M3-CU03) si existe resultado final de sacrificio sincronizado o si la población actual sincronizada es 0, y no existe Liquidación `ACTIVA`; y la acción de consultar el Desglose (M3-CU05) si existe una Liquidación para el lote.
...
- **SC-003**: Cero accesos permitidos a la generación de la Liquidación fuera de `Vaciado Sanitario`; la Liquidación sin resultado de sacrificio solo se permite en siniestro total con población actual igual a 0.
